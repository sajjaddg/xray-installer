...
export default function App({ type }: AppProps) {
  ...
  useEffect(() => {
    const configure = async () => {
      try {
        ...
        // Step 2: Install Xray
        setStep(2);
        execSync('bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install --beta');
        ...
        fs.writeFileSync('/usr/local/etc/xray/config.json', JSON.stringify(updatedConfig, null, 2));
  ...
  return (
    <Box flexDirection="column" padding={1}>
      <Text bold color="green">🚀 Xray Reality Installer</Text>
      ...
      {url && (
        <Box marginTop={1} flexDirection="column">
          <Text bold color="green">✅ Configuration Completed!</Text>
